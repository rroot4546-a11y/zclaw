          ...
          >
            <template #left>
              <span class="thread-left-stack">
                ...
                <button class="thread-pin-button" type="button" :title="t('pinAction')" @click="togglePin(thread.id)">
            ...
            <template #right-hover>
              ...
              >
                <span v-if="archiveConfirmThreadId === thread.id">{{ t('confirm') }}</span>
...
function getNewThreadButtonAriaLabel(projectName: string): string {
  return `${t('newThreadIn')} ${getProjectDisplayName(projectName)}`
}
