  ...
  <DesktopLayout :is-sidebar-collapsed="isSidebarCollapsed">
    <template #sidebar>
      <section class="sidebar-root">
        <div v-if="!isSidebarCollapsed" class="zclaw-brand">
          <span class="zclaw-brand-mark">z</span>
          <span class="zclaw-brand-name">zclaw</span>
        </div>
...
const contentTitle = computed(() => {
  if (isHomeRoute.value) return t('newThread')
  return selectedThread.value?.title ?? t('chooseAThread')
})
const autoRefreshButtonLabel = computed(() =>
  isAutoRefreshEnabled.value
    ? `${t('autoRefreshIn')} ${String(autoRefreshSecondsLeft.value)}s`
    : t('enableAutoRefresh'),
)
